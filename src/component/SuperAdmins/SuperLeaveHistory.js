...
const MinistryLeaveHistory = () => {
  ...
  const [isSuperAdmin, setIsSuperAdmin] = useState(false); // Flag to check if the user is super admin

  useEffect(() => {
    // Fetch user's access level from the `access_level` table
    const checkUserAccess = async () => {
      ...
      if (error) {
        console.error("Error fetching user access level:", error.message);
      } else {
        // Set true if user is a super admin (access_id = 3)
        setIsSuperAdmin(data.access_id === 3);
      }
    };
    checkUserAccess();
  }, []);
  ...
  useEffect(() => {
    const fetchLeaveHistory = async () => {
      ...
      // Fetch leave data with or without department filter based on super admin status
    ...
    fetchLeaveHistory();
  }, [departmentId, isSuperAdmin]); // Triggered when departmentId or isSuperAdmin changes
