...
const ChatBox = ({ selectedUser, onClose }) => {
  ...
  }, [selectedUser.employee_id]); // Adjust dependencies to trigger correctly
  ...
  const sendMessage = async () => {
    ...
    try {
      const { error } = await supabase.from("direct_chats").insert([
        {
          sender_id: currentUserId,            // ID of the current user (sender)
          receiver_id: selectedUser.employee_id,  // ID of the selected user (receiver)
          message: encryptedMessage,            // The encrypted message to send
