...
import { FaCalendarAlt, FaUser, FaHome } from "react-icons/fa"; // Calendar, User, and Home Icons
...
import { FaTasks, FaBell } from "react-icons/fa"; // Tasks and Notification Icons
import { CheckCircleIcon } from "@heroicons/react/24/outline"; // Checkin Icon
...
const QuickActionPopup = () => {
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      if (session) {
        setIsLoggedIn(true); // If session exists, user is logged in
      } else {
        setIsLoggedIn(false); // If no session, user is not logged in
      }
    };
    checkSession();
  }, []);
  ...
  return (
    <div className="relative">
      ...
      {isOpen && (
        <div
          ref={popupRef}
          className="fixed top-1/2 right-6 transform -translate-y-1/2 bg-white shadow-md rounded-lg w-64 z-50"
        >
          {isLoggedIn ? (
            <div className="grid grid-cols-3 gap-4 p-4">
              ...
              <button
                onClick={() => navigate("/")}
                className="flex flex-col items-center space-y-2 text-gray-700 hover:text-blue-600"
              >
            ...
            <div className="flex flex-col items-center p-4 space-y-4">
              <p className="text-gray-700 text-sm">
                You have to sign up to access this feature
              </p>
              <button
